...
void fill_clean_list(t_list *list, t_myshell **c_list, t_env *list_env)
{
	...
	while(list)
	{
		...
		{
			list = list->next;
			ft_lstadd_back_outfile(&list_outfile, ft_lstnew_outfile(get_string(&list, list_env)));
		}
		else if(list->next  && (list->trag == TOKEN_LESS_THAN))
		{
			list = list->next;
			ft_lstadd_back_infile(&list_infile, ft_lstnew_infile(get_string(&list, list_env)));
		}
		else
			ft_lstadd_back_args(&list_arg, ft_lstnew_args(get_string(&list, list_env)));
		// printf("%s\n", list->data);
